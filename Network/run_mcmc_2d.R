...
for (i in 1:(R + B)){
  ...
  if (ind == FALSE){
    ...
    c = 1 - sum(rho[-1]^2) + prod(rho[3:4])^2
     ...
     mn <- min(end) + 0.001
  mx <- max(end) - 0.001
  if(f(mn) <0 | f(mx) > 0) {hat <- 0} else {hat <- uniroot(f,c(mn,mx))$root}
  sd <- sqrt(-h^2 / (log_pyv(y,yprim,v,J,mu,theta,phi,sigma_v,c(hat+h,rho[2:4])) - 
2*log_pyv(y,yprim,v,J,mu,theta,phi,sigma_v,c(hat,rho[2:4])) + 
log_pyv(y,yprim,v,J,mu,theta,phi,sigma_v,c(hat-h,rho[2:4]))))
sd <- ifelse(is.nan(sd),0.05,sd)  
rho[1] <- update_rho(y,yprim,v,J,mu,theta,phi,sigma_v,rho,hat,sd,0)
  ...
  mx <- max(end) - 0.001
  if(f(mn) <0 | f(mx) > 0) {hat <- 0} else {hat <- uniroot(f,c(mn,mx))$root}
  sd <- sqrt(-h^2 / (log_pyv(y,yprim,v,J,mu,theta,phi,sigma_v,c(rho[1],hat+h,rho[3:4])) - 
2*log_pyv(y,yprim,v,J,mu,theta,phi,sigma_v,c(rho[1],hat,rho[3:4])) + 
log_pyv(y,yprim,v,J,mu,theta,phi,sigma_v,c(rho[1],hat-h,rho[3:4]))))
sd <- ifelse(is.nan(sd),0.05,sd)  
rho[2] <- update_rho(y,yprim,v,J,mu,theta,phi,sigma_v,rho,hat,sd,1)
  ...
  mx <- max(end) - 0.001
  if(f(mn) <0 | f(mx) > 0) {hat <- 0} else {hat <- uniroot(f,c(mn,mx))$root}
  sd <- sqrt(-h^2 / (log_pyv(y,yprim,v,J,mu,theta,phi,sigma_v,c(rho[1:2],hat+h,rho[4])) - 
2*log_pyv(y,yprim,v,J,mu,theta,phi,sigma_v,c(rho[1:2],hat,rho[4])) + 
log_pyv(y,yprim,v,J,mu,theta,phi,sigma_v,c(rho[1:2],hat-h,rho[4]))))
sd <- ifelse(is.nan(sd),0.05,sd)  
rho[3] <- update_rho(y,yprim,v,J,mu,theta,phi,sigma_v,rho,hat,sd,2)
  ...
  mx <- max(end) - 0.001
  if(f(mn) <0 | f(mx) > 0) {hat <- 0} else {hat <- uniroot(f,c(mn,mx))$root}
  sd <- sqrt(-h^2 / (log_pyv(y,yprim,v,J,mu,theta,phi,sigma_v,c(rho[1:3],hat+h)) - 
2*log_pyv(y,yprim,v,J,mu,theta,phi,sigma_v,c(rho[1:3],hat)) + 
log_pyv(y,yprim,v,J,mu,theta,phi,sigma_v,c(rho[1:3],hat-h))))
sd <- ifelse(is.nan(sd),0.05,sd)  
