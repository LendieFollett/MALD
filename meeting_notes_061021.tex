...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in getSymbols("{}BTC-USD"{}, from = "{}2014-09-15"{}, to = "{}2020-09-30"{}): could not find function "{}getSymbols"{}}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in as.data.frame(`BTC-USD`): object 'BTC-USD' not found}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in BTC\$Date <- seq(as.Date("{}2014-09-17"{}), as.Date("{}2020-09-30"{}), : object 'BTC' not found}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in BTC\$`BTC-USD.Close`[BTC\$Date == "{}2020-04-17"{}] <- 7096.18: object 'BTC' not found}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in getSymbols("{}\textasciicircum{}GSPC"{}, from = "{}2014-09-15"{}, to = "{}2020-09-30"{}): could not find function "{}getSymbols"{}}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in as.data.frame(GSPC): object 'GSPC' not found}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in rownames(SP500): object 'SP500' not found}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in merge(BTC, SP500): object 'BTC' not found}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in nrow(S): object 'S' not found}}\end{kframe}
\end{knitrout}

\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{#Posterior Means}
\hlkwd{lapply}\hlstd{(keepsBTCSP0[}\hlkwd{c}\hlstd{(}\hlnum{4}\hlstd{,}\hlnum{6}\hlopt{:}\hlnum{17}\hlstd{)], domean,} \hlkwc{total} \hlstd{=} \hlnum{10000}\hlstd{)} \hlopt{%>%} \hlkwd{str}\hlstd{()} \hlcom{#diffuse priors}
\end{alltt}
\begin{verbatim}
## List of 13
##  $ lambda  : num [1:4] 0.1156 0.0398 0.0469 0.7977
##  $ sigma_v : num [1:2] 0.441 0.202
##  $ sigma_c : num [1:2] 9.02 0.71
##  $ rhoc    : num -0.0308
##  $ xi_cw   : num [1:2] -2.23 -1.02
##  $ xi_y1eta: num 7.63
##  $ xi_y1w  : num 0.566
##  $ xi_y2eta: num 0.358
##  $ xi_y2w  : num -0.165
##  $ phi     : num [1:2] 0.984 0.98
##  $ theta   : num [1:2] 7.03 1.22
##  $ mu      : num [1:2] 0.175 0.0489
##  $ rho     : num [1:4] 0.028 0.0456 0.4226 -0.7189
\end{verbatim}
\begin{alltt}
\hlkwd{lapply}\hlstd{(keepsBTCSP[}\hlkwd{c}\hlstd{(}\hlnum{4}\hlstd{,}\hlnum{6}\hlopt{:}\hlnum{17}\hlstd{)], domean,} \hlkwc{total} \hlstd{=} \hlnum{20000}\hlstd{)} \hlopt{%>%} \hlkwd{str}\hlstd{()} \hlcom{#fulop priors 1}
\end{alltt}
\begin{verbatim}
## List of 13
...
\end{verbatim}
\begin{alltt}
\hlkwd{lapply}\hlstd{(keepsBTCSP2[}\hlkwd{c}\hlstd{(}\hlnum{4}\hlstd{,}\hlnum{6}\hlopt{:}\hlnum{17}\hlstd{)], domean,} \hlkwc{total} \hlstd{=} \hlnum{20000}\hlstd{)} \hlopt{%>%} \hlkwd{str}\hlstd{()} \hlcom{#fulop priors 2}
\end{alltt}
\begin{verbatim}
## List of 13
##  $ lambda  : num [1:4] 0.1027 0.0361 0.0803 0.7809
##  $ sigma_v : num [1:2] 0.435 0.213
##  $ sigma_c : num [1:2] 0.739 0.17
##  $ rhoc    : num 0.0455
##  $ xi_cw   : num [1:2] -5.828 -0.165
##  $ xi_y1eta: num 0.946
##  $ xi_y1w  : num 4.57
##  $ xi_y2eta: num 0.653
##  $ xi_y2w  : num -0.659
##  $ phi     : num [1:2] 0.985 0.976
##  $ theta   : num [1:2] 6.96 1.11
##  $ mu      : num [1:2] 0.1591 0.0507
##  $ rho     : num [1:4] 0.00894 0.07028 0.4568 -0.70367
\end{verbatim}
\begin{alltt}
\hlkwd{rm}\hlstd{(keepsBTCSP0);}\hlkwd{rm}\hlstd{(keepsBTCSP2)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
{\ttfamily\noindent\itshape\color{messagecolor}{\#\# `stat\_bin()` using `bins = 30`. Pick better value with `binwidth`.}}\end{kframe}
\includegraphics[width=6in,height=5in]{figure/unnamed-chunk-4-1} 
\end{knitrout}
...
{\ttfamily\noindent\itshape\color{messagecolor}{\#\# `stat\_bin()` using `bins = 30`. Pick better value with `binwidth`.}}\end{kframe}
\includegraphics[width=5in,height=5in]{figure/unnamed-chunk-5-1} 
\end{knitrout}
...
\hlstd{keepsBTCSP}\hlopt{$}\hlstd{delta} \hlopt{%>%}
  ...
  \hlkwd{ggplot}\hlstd{()} \hlopt{+}
  \hlkwd{geom_bar}\hlstd{(}\hlkwd{aes}\hlstd{(}\hlkwc{x} \hlstd{= Var2,} \hlkwc{fill} \hlstd{= value,} \hlkwc{colour} \hlstd{= value),} \hlkwc{position} \hlstd{=} \hlstr{"fill"}\hlstd{)} \hlopt{+}
  \hlkwd{scale_fill_brewer}\hlstd{(}\hlstr{"Type"}\hlstd{,}\hlkwc{type} \hlstd{=} \hlstr{"qual"}\hlstd{)} \hlopt{+}
    \hlkwd{scale_colour_brewer}\hlstd{(}\hlstr{"Type"}\hlstd{,}\hlkwc{type} \hlstd{=} \hlstr{"qual"}\hlstd{)} \hlopt{+}
  \hlkwd{labs}\hlstd{(}\hlkwc{x} \hlstd{=} \hlstr{"Time"}\hlstd{,} \hlkwc{y} \hlstd{=} \hlstr{"Proportion"}\hlstd{)}
\end{alltt}
\end{kframe}
\includegraphics[width=.95\linewidth,height=4in]{figure/unnamed-chunk-6-1} 
\begin{kframe}\begin{alltt}
\hlcom{#it's worth noting the MVN plot looks almost identical - same patterns}

\hlcom{#proportion of dates that had each of the classes}
\hlkwd{apply}\hlstd{(keepsBTCSP}\hlopt{$}\hlstd{delta,} \hlnum{2}\hlstd{, Mode)} \hlopt{%>%} \hlkwd{table}\hlstd{()}
\end{alltt}
\begin{verbatim}
## .
##    0    1    2    3 
